...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_string_input($field_info,$value)
	{
		...
		$extra_attributes = ' style="color:#000;"';
		if (!empty($field_info->db_max_length)) {
		...
		$input = "<input id='field-{$field_info->name}' class='form-control wew' name='{$field_info->name}' type='text' value=\"$value\" $extra_attributes />";
		return $input;
	}
